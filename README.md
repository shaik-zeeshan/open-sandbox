...
- `SANDBOX_SECRETS_KEY` (optional): key used to encrypt sandbox secret env values at rest; must be a raw 32-byte string, 64-character hex string, or base64-encoded 32-byte key, and should be left unset unless you are explicitly configuring secret env support
...
Preview routes generated by open-sandbox (for sandboxes, direct containers, and compose services) can be customized per port or per compose service. All customization is applied by Traefik as middleware on the preview route — the upstream container itself does not need to change.

### Supported options

| Field | Description |
|---|---|
| `request_headers` | Key/value headers injected into every proxied request |
| `response_headers` | Key/value headers added to every proxied response |
| `cors` | CORS / security header settings (see below) |
| `path_prefix_strip` | Strip a path prefix before forwarding upstream (Traefik `stripPrefix`) |
| `skip_auth` | Bypass sandbox forward-auth; useful for public endpoints |

CORS sub-fields: `allow_origins` (list), `allow_methods` (list), `allow_headers` (list), `allow_credentials` (bool), `max_age` (int, seconds).

### Compose service proxy config

Add an `x-open-sandbox.proxy` block under any service that needs customization. Services without the extension use the default platform middleware (forward-auth + preview header injection).
...
Pass this compose YAML to `POST /api/compose/up` as the `content` field. The server extracts the `x-open-sandbox.proxy` data and writes per-service Traefik middleware automatically.

### Sandbox per-port proxy config

When creating a sandbox via `POST /api/sandboxes`, include a `proxy_config` map keyed by the **string representation** of each private port number:
...
`proxy_config` is persisted with the sandbox record and applied to the preview Traefik route every time routes are synced. Each key is the container's private port number as a string.

### Limitations

- Proxy config applies **only to preview routes** served by Traefik. It has no effect on direct host port access.
...
- Compose YAML normalization: the YAML is re-serialized when stored; formatting and comments inside `x-open-sandbox.proxy` blocks are not preserved.
- Only ports **published to the host** (`HOST:CONTAINER`) are previewable. Internal-only ports are not routed.
